use std::ops::Add;
use chrono::{Days, NaiveDate, TimeDelta, Utc, Weekday};
use chrono_tz::Tz;
...
async fn main() {
    ...
    let today = Utc::now().with_timezone(&Tz::Europe__Helsinki).date_naive();
    let friday = |d: NaiveDate| d.week(Weekday::Mon).first_day().add(Days::new(FRIDAY_OFFSET));
        ...
        .iter()
    {
        let start_date = NaiveDate::parse_from_str(
            &week_menu["date"]["start"].as_str().unwrap(),
            "%Y-%m-%d",
        ).unwrap();
